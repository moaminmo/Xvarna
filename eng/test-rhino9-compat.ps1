...
if (-not $InProcess) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Rhino 9 isolated compatibility process failed with exit code $LASTEXITCODE."
    }
    return
}
...
if ([string]::IsNullOrWhiteSpace($RhinoRoot)) {
    throw "Rhino 9 was not found. Checked the release and WIP installation roots."
}
...
foreach ($path in @($rhinoCommon, $grasshopperIo, $grasshopper, $managedConnector, $grasshopperConnector)) {
    if (-not (Test-Path -LiteralPath $path)) {
        throw "Required Rhino 9 compatibility-test file is missing: $path"
    }
}
...
if ($missing.Count -ne 0) {
    throw "Rhino 9 compatibility load is missing component(s): $($missing -join ', ')"
}
if ($components.Count -ne $expectedNicknames.Count) {
    throw "Rhino 9 compatibility load expected $($expectedNicknames.Count) components but found $($components.Count)."
}
$missingIcons = @($components | Where-Object { $null -eq $_.Icon_24x24 })
if ($missingIcons.Count -ne 0) {
    throw "Rhino 9 compatibility load found components without product icons: $($missingIcons.NickName -join ', ')"
}
$duplicateGuids = @(
    $components |
...
if ($duplicateGuids.Count -ne 0) {
    throw "Rhino 9 compatibility load found duplicate component GUIDs."
}

$components |
    Sort-Object Category, SubCategory, NickName |
    ForEach-Object { Write-Host "$($_.NickName) | $($_.Category) > $($_.SubCategory) | $($_.ComponentGuid)" }
Write-Host "Rhino 9 runtime compatibility passed for $($components.Count) XVARNA components from $RhinoRoot."
